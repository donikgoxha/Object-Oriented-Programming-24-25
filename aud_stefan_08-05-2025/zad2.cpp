...
public:
    QuizAttempt(const char *ID = "235003") {
        strcpy(this->ID, ID);
    }
...
public:
    MCQuizAttempt(const char *ID, const char *correct, const char *student): QuizAttempt(ID) {
...
public:
    TFQuizAttempt(const char *ID, bool *correct, bool *student) : QuizAttempt(ID) {
        for (int i = 0; i < 10; i++) {
